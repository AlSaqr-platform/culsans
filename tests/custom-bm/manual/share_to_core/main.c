...
volatile uint8_t share [CACHE_SIZE_BYTE] __attribute__((section(".cache_share_region")));
...
void thread_entry(int cid, int nc)
{
  ...
  cacheline_t *share_block = (cacheline_t*)(share + 2048);
  cacheline_t *dummy_block = (cacheline_t*)(dummy + 2048);

  test(cid, nc, share_block, dummy_block);
